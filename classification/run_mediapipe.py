...
ntu_to_mp = dict()
for key, value in mp_to_ntu.items():
    ntu_to_mp[value] = key
...
out = cv2.VideoWriter("output/output2.mp4", fourcc, 25, (frame.shape[1], frame.shape[0]))
...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            ...
            for i, keypoint in enumerate(results.pose_landmarks.landmark):
                if i in mp_to_ntu:
                    keypoints[mp_to_ntu[i]] = [keypoint.x, keypoint.y, keypoint.z]
            ...
            cv2.putText(image, str(result), (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2, cv2.LINE_AA)
